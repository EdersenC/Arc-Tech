...
`WorkflowService.getOrCreateForOrchestration(projectId, orchestrationId, goal)` creates a revision `0` empty graph when no graph exists yet. The planner is responsible for populating the first-class goal, requirement, architecture, task, and open-question nodes with semantic workflow patches derived from the user's orchestration prompt. `WorkflowService.applyPlannerPatch(projectId, orchestrationId, patch)` validates the patch, checks project/orchestration ownership, applies it with `applyWorkflowPatch`, updates the snapshot, and records the patch history.
