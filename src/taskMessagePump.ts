...
export class TaskMessagePump {
  ...
  private async createPullRequestIfPossible(task: Task, finalSummary: string, diffStat: string): Promise<string | null> {
    ...
    try {
      ...
      const title = pullRequestTitle(task, finalSummary);
      return await this.githubPrService.createPrForTask(
        task,
        title,
        prBody(task, finalSummary),
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
...
function prBody(task: Task, finalSummary: string): string {
...
Generated by Arc-Tech for task ${taskLabel(task)}.`;
}

function pullRequestTitle(task: Task, finalSummary: string): string {
  const agentTitle = extractPullRequestTitle(finalSummary) ?? extractPullRequestTitle(task.prompt);
  return agentTitle ?? `Codex task ${taskDisplayNumber(task)}: ${oneLine(task.prompt, 72)}`;
}

function extractPullRequestTitle(value: string): string | null {
  const match = /(?:^|\n)\s*(?:[-*]\s*)?(?:suggested\s+)?(?:pr|pull request)\s+title\s*:\s*([^\r\n]+)/i.exec(value);
  if (!match) {
    return null;
  }
  return sanitizePullRequestTitle(match[1]);
}

function sanitizePullRequestTitle(value: string): string | null {
  const cleaned = value
    .replace(/[`*_~]/g, "")
    .replace(/^\s*["']|["']\s*$/g, "")
    .replace(/[\u0000-\u001f\u007f]/g, " ")
...
function buildCompletionSummary(agentSummary: string, pullRequestUrl: string | null): string {
  ...
  if (!pullRequestUrl) {
    return agentNotes || "Codex completed. The runner committed local task changes when present; no PR URL is available.";
  }
...
function sanitizeAgentSummary(summary: string): string {
  ...
  text = text.replace(/(?:^|\n)Commit\/PR:\s*[\s\S]*?(?=\n\n|Files changed|Tests|Known risks|Task|Diff stat|Generated by|##|$)/i, "\n");
  text = text.replace(
    /(?:^|\n)(?:Could not|Unable to)\s+(?:commit|push|create\s+(?:a\s+)?pr|open\s+(?:a\s+)?pull request)[\s\S]*?(?:sandbox|read-only file system|index\.lock|git metadata)[\s\S]*?(?=\n\n|Files changed|Tests|Known risks|Task|Diff stat|Generated by|##|$)/i,
    "\n",
  );
  text = text.replace(/(?:^|\n)Result:\s*[\s\S]*?(?=\n(?:Task|Diff stat|Generated by|##|$))/i, "\n");
  ...
  const lines = text
    ...
    .filter((line) => {
      ...
      if (/^(?:suggested\s+)?(?:pr|pull request)\s+title\s*:/i.test(trimmed)) return false;
      ...
      if (/could not .*git write|could not commit|could not push|git add\/commit failed|unable to create .*index\.lock|read-only file system/i.test(trimmed)) return false;
      return true;
    })
    .join("\n");
