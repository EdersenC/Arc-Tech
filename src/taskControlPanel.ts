...
type TaskButtonAction = "start" | "cancel" | "diff" | "merge" | "abandon" | "ask";
...
export class TaskControlPanelService {
  ...
  async handleButton(interaction: ButtonInteraction): Promise<boolean> {
    ...
    await interaction.editReply(`Send a normal message in <#${task.discordThreadId ?? interaction.channelId}> to ask Codex a follow-up.`);
    return true;
  }
  ...
  private async loadAuthorizedTask(interaction: ButtonInteraction, taskId: number): Promise<Task | null> {
  ...
  private isAllowed(interaction: ButtonInteraction | StringSelectMenuInteraction, task: Task): boolean {
...
function buttonRows(task: Task, forceDisabled = false): ActionRowBuilder<ButtonBuilder>[] {
  ...
  return [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`task:start:${task.id}`).setLabel("Start").setStyle(ButtonStyle.Success).setDisabled(startDisabled),
      new ButtonBuilder().setCustomId(`task:cancel:${task.id}`).setLabel("Cancel").setStyle(ButtonStyle.Danger).setDisabled(disabled),
      new ButtonBuilder().setCustomId(`task:diff:${task.id}`).setLabel("Show Diff").setStyle(ButtonStyle.Secondary).setDisabled(forceDisabled),
      new ButtonBuilder().setCustomId(`task:ask:${task.id}`).setLabel("Ask Codex").setStyle(ButtonStyle.Primary).setDisabled(disabled),
    ),
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`task:merge:${task.id}`).setLabel("Merge").setStyle(ButtonStyle.Primary).setDisabled(mergeDisabled),
      new ButtonBuilder().setCustomId(`task:abandon:${task.id}`).setLabel("Abandon").setStyle(ButtonStyle.Secondary).setDisabled(disabled),
    ),
  ];
}
...
function parseButtonCustomId(customId: string): { action: TaskButtonAction; taskId: number } | null {
  const match = /^task:(start|cancel|diff|merge|abandon|ask):(\d+)$/.exec(customId);
  if (!match) {
    return null;
  }
